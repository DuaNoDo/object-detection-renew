"""
Mask R-CNN
Train on the toy plane dataset and implement color splash effect.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 plane.py train --dataset=/path/to/plane/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 plane.py train --dataset=/path/to/plane/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 plane.py train --dataset=/path/to/plane/dataset --weights=imagenet

    # Apply color splash to an image
    python3 plane.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 plane.py splash --weights=last --video=<URL or path to file>
"""
       ...
       Returns:
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            if p['name'] == "polygon":
                ...
                w = p['width']
                h = p['height']
                rr, cc = skimage.draw.polygon([y, y, y + h, y + h], [x, x + w, x + w, x])
            mask[rr, cc, i] = 1
